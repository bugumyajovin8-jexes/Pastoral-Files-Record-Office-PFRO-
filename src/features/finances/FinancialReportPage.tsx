        ...
        <div className="flex bg-[var(--color-surface)]/10 p-1 rounded-xl mt-1.5 self-start overflow-x-auto max-w-full">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer whitespace-nowrap ${
              activeTab === 'muhtasari' 
                ? 'bg-[var(--color-surface)] text-[var(--color-teal-on)] shadow-sm' 
        ...
        <div className="bg-[var(--color-surface)] rounded-2xl p-3 border border-[var(--color-border)] shadow-sm flex flex-col gap-2">
          <div className="flex items-center gap-1.5">
            <div className="flex items-center gap-1 flex-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg px-2 py-1.5">
              <Calendar size={13} className="text-[var(--color-teal-on)]" />
              <select
                value={timeRange}
          ...
          {timeRange === 'custom' && (
            <div className="grid grid-cols-2 gap-2 mt-1 p-2 bg-[#F4EFFF] rounded-xl border border-[#DFCEFD] animate-fadeIn">
              <div className="flex flex-col">
                <span className="text-[11px] font-bold text-[#5A17D7] mb-1">Tarehe ya Kuanza:</span>
                <input 
                  type="date" 
                  value={customStartDate} 
                  onChange={(e) => setCustomStartDate(e.target.value)}
                  className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-md p-1.5 text-[12px] font-bold focus:outline-none focus:border-[#00C9A7]"
                />
              </div>
              <div className="flex flex-col">
                <span className="text-[11px] font-bold text-[#5A17D7] mb-1">Tarehe ya Mwisho:</span>
                <input 
                  type="date" 
                  value={customEndDate} 
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="p-3 bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-sm relative overflow-hidden">
                    ...
                    <span className="text-[11px] text-[var(--color-teal-on)] font-extrabold block uppercase tracking-tight">Zaka</span>
                    <p className="text-[14px] font-black text-[var(--color-text)] mt-1">
                      {report.zakaTotal.toLocaleString()}
                    </p>
                    <span className="text-[11px] text-[var(--color-text-muted)] font-semibold block mt-0.5">TZS</span>
                  </div>

                  <div className="p-3 bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1.5 h-full bg-[#F37B1D]" />
                    <span className="text-[11px] text-[#F37B1D] font-extrabold block uppercase tracking-tight">Sadaka</span>
                    <p className="text-[14px] font-black text-[var(--color-text)] mt-1">
                  ...
                  <div className="p-3 bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-sm relative overflow-hidden">
                    ...
                    <span className="text-[11px] text-[#0074DB] font-extrabold block uppercase tracking-tight">Kategoria Nyingine</span>
                    <p className="text-[14px] font-black text-[var(--color-text)] mt-1">
                ...
                <div className="bg-[var(--color-ink)] rounded-[24px] p-5 text-[var(--color-text)] border border-[#4314A8] shadow-md relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-[12px] bg-[var(--color-surface)] text-[var(--color-teal-on)] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider">
                      Uchambuzi wa Formula ya Mgawanyo
                    </span>
                    <span className="text-[12px] text-[var(--color-text-dim)] font-semibold">{rangeLabelSwahili()}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3.5 border-t border-white/10 pt-4">
                    ...
                    <div className="flex justify-between items-center bg-[var(--color-surface)]/5 hover:bg-[var(--color-surface)]/10 p-3 rounded-xl transition-all border border-white/5">
                      <div>
                        <span className="text-[11px] text-[var(--color-text-dim)] font-medium block uppercase tracking-wider">Jumla ya Fedha ya Field</span>
                        <span className="text-[12px] text-[var(--color-teal-on)] font-bold">Zaka + (58% ya Sadaka)</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[17px] font-black text-[var(--color-teal-on)] font-mono block">
                          TZS {globalFieldFunds.toLocaleString()}
                        </span>
                        <span className="text-[11px] text-gray-300">({report.zakaTotal.toLocaleString()} + {(report.sadakaTotal * 0.58).toLocaleString()})</span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-[var(--color-surface)]/5 hover:bg-[var(--color-surface)]/10 p-3 rounded-xl transition-all border border-white/5">
                      <div>
                        <span className="text-[11px] text-[var(--color-text-dim)] font-medium block uppercase tracking-wider">Jumla ya Fedha ya Kanisa (Mahali)</span>
                        <span className="text-[12px] text-[#FFAC62] font-bold">42% ya Sadaka tu</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[17px] font-black text-[#FFAC62] font-mono block">
                          TZS {globalChurchFunds.toLocaleString()}
                        </span>
                        <span className="text-[11px] text-gray-300">({(report.sadakaTotal * 0.42).toLocaleString()})</span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-[var(--color-surface)]/10 p-3.5 rounded-xl border border-white/20 mt-1">
                      <div>
                        ...
                        <span className="text-[11px] text-gray-300 italic">Mgawanyo huu halisi wa Zaka + Sadaka</span>
                      </div>
                      <div className="text-right">
                        ...
                        <span className="text-[11px] text-gray-300">Sawa na Jumla kuu TZS {(report.zakaTotal + report.sadakaTotal).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[var(--color-surface)] rounded-2xl p-4 border border-[var(--color-border)] shadow-sm flex flex-col gap-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[12px] font-bold text-[var(--color-text-muted)] font-mono">
                      ...
                      Object.entries(report.otherCategories).map(([catName, catAmount]) => (
                        <div key={catName} className="flex justify-between items-center bg-[var(--color-surface)] px-3 py-2.5 rounded-xl border border-[var(--color-border)] hover:shadow-xs transition-shadow">
                          <div>
                            ...
                            <span className="text-[11px] text-[var(--color-text-muted)] uppercase tracking-wider font-semibold">Kategoria Maalum</span>
                          </div>
                          <span className="text-[12px] font-mono font-bold text-gray-950">TZS {catAmount.toLocaleString()}</span>
                        </div>
                      ))
                    ) : (
                      <p className="text-[12px] text-[var(--color-text-muted)] text-center py-4">Hakuna michango ya ziada iliyopatikana mbali na kategoria kuu.</p>
                    )}
                  </div>
                </div>

                {/* Ultimate Income Total */}
                <div className="p-4 rounded-2xl border border-[color-mix(in_srgb,var(--color-teal)_28%,transparent)] bg-[color-mix(in_srgb,var(--color-teal)_12%,transparent)] flex justify-between items-center">
                  <div>
                    <span className="text-[12px] text-[var(--color-teal-on)] font-bold uppercase tracking-wider block">Jumla Kuu ya Mapato Yote</span>
                    <span className="text-[12px] text-[var(--color-text-dim)] font-medium leading-none">Inajumuisha Formula zote + Mapato mengine yote</span>
                  </div>
                  <span className="text-[18px] font-black text-[var(--color-teal-on)] font-mono shrink-0">
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                      historyPeriodType === 'monthly'
                        ...
                        : 'text-[var(--color-teal-on)] hover:bg-[var(--color-surface)]/50'
                    }`}
                  >
                          ...
                          >
                            ...
                            <div className="flex justify-between items-start pl-2">
                              <div>
                                ...
                                <span className="text-[11px] font-bold text-purple-700 bg-purple-50 border border-purple-100 px-2.5 py-0.5 rounded-lg mt-1 inline-block uppercase tracking-wider">
                                  {item.count} {item.count === 1 ? 'Mchango' : 'Michango'}
                                </span>
                              </div>
                              <div className="text-right font-mono">
                                <span className="text-[14px] font-black text-[var(--color-teal-on)] block">
                                  TZS {grandTotal.toLocaleString()}
                                </span>
                                <span className="text-[11px] text-[var(--color-text-muted)] uppercase tracking-widest font-black leading-none block mt-0.5">Jumla Kuu</span>
                              </div>
                            </div>

                            {/* Contribution Sources */}
                            <div className="grid grid-cols-3 gap-2 pl-2 text-[12px] bg-[var(--color-surface)]/50 p-2.5 rounded-xl border border-[var(--color-border)]">
                              <div>
                                <span className="text-[var(--color-text-muted)] font-extrabold block uppercase tracking-tight text-[11px]">Zaka</span>
                                <strong className="text-[var(--color-text)] font-black font-mono">TZS {totalZaka.toLocaleString()}</strong>
                              </div>
                              <div>
                                <span className="text-[var(--color-text-muted)] font-extrabold block uppercase tracking-tight text-[11px]">Sadaka</span>
                                <strong className="text-[var(--color-text)] font-black font-mono">TZS {totalSadaka.toLocaleString()}</strong>
                              </div>
                              <div>
                                <span className="text-[var(--color-text-muted)] font-extrabold block uppercase tracking-tight text-[11px]">Zinginezo</span>
                            ...
                            <div className="grid grid-cols-2 gap-2 pl-2">
                              <div className="bg-[#EFFFFA] p-2.5 rounded-xl border border-[#D5FAF0]">
                                <span className="text-[11px] text-teal-800 font-extrabold block uppercase tracking-tight">🏢 Mgawo wa Field (Zaka + 58% Sadaka)</span>
                              ...
                              <div className="bg-[#FFF8F3] p-2.5 rounded-xl border border-[#FFE8DA]">
                                <span className="text-[11px] text-orange-850 font-extrabold block uppercase tracking-tight">⛪ Mgawo wa Kanisa (42% Sadaka)</span>
