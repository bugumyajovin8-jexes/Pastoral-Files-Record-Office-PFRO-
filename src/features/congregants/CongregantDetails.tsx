...
import { maritalLabel } from "../../lib/memberFields";
import {
  LineChart,
  Line,
  BarChart,
...
function InfoItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex gap-3 items-center py-2 border-b border-[var(--color-border)] last:border-0 last:pb-0">
      <div className="text-[var(--color-violet-on)]">{icon}</div>
      <div>
...
export default function CongregantDetails() {
  ...
  const handleDelete = async () => {
    const name = congregant?.full_name || 'mshiriki huyu';

    // `contributions.congregant_id` references this row, so deleting a member
    // who has given anything fails on a foreign key. The old version ignored
    // the error and navigated away as if it had worked.
    if (contributions.length > 0) {
      alert(
        `Huwezi kumfuta ${name} kwa sababu ana kumbukumbu ${contributions.length} za michango. ` +
        `Kumbukumbu za fedha lazima zihifadhiwe.`
      );
      return;
    }

    if (!confirm(`Una uhakika unataka kumfuta ${name}? Hatua hii hairudishwi.`)) return;

    const { error } = await supabase.from('congregants').delete().eq('id', id);
    if (error) {
      alert('Imeshindikana kumfuta mshiriki: ' + error.message);
      return;
    }
  ...
  if (hasAccess === false) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] bg-[var(--color-surface)] rounded-3xl m-6 p-8 border border-[var(--color-border)] shadow-sm text-center font-sans">
        <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4 font-bold text-lg text-[var(--color-rose-on)]" style={{background:'color-mix(in srgb, var(--color-rose) 14%, transparent)'}}>!</div>
    ...
    <div className="min-h-screen bg-[var(--color-surface)] pb-20">
      ...
      <div className="px-4 space-y-6 pt-6">
        ...
        <div className="bg-[var(--color-surface)] p-5 rounded-2xl shadow-sm border border-[var(--color-border)]">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-6 pb-3 border-b border-[var(--color-border)]">
                ...
                <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 border ${
                        activeCategory === 'all'
                            ? 'bg-[var(--color-ink-600)] text-[var(--color-text)] border-[var(--color-ink-500)] shadow-sm'
                            : 'bg-[var(--color-surface)] text-[var(--color-text-dim)] border-[var(--color-border)] hover:bg-[var(--color-surface-2)]'
                    }`}
                >
                ...
                <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 border ${
                        activeCategory === 'zaka'
                            ? 'bg-[color-mix(in_srgb,var(--color-teal)_10%,transparent)] text-[var(--color-teal-on)] border-[color-mix(in_srgb,var(--color-teal)_35%,transparent)]'
                            : 'bg-[var(--color-surface)] text-[var(--color-text-dim)] border-[var(--color-border)] hover:bg-[var(--color-surface-2)]'
                    }`}
                >
                ...
                <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 border ${
                        activeCategory === 'sadaka'
                            ? 'bg-[color-mix(in_srgb,var(--color-gold)_10%,transparent)] text-[var(--color-gold-on)] border-[color-mix(in_srgb,var(--color-gold)_35%,transparent)]'
                            : 'bg-[var(--color-surface)] text-[var(--color-text-dim)] border-[var(--color-border)] hover:bg-[var(--color-surface-2)]'
                    }`}
                >
            ...
            {chartData.length > 0 ? (
                ...
                <div className="py-12 text-center bg-[var(--color-surface)] rounded-xl border border-dashed border-[var(--color-border)]">
                    ...
                    <p className="text-[12px] text-[var(--color-text-muted)] mt-1">Bofya "Ongeza Mchango" hapo chini kurekodi mchango wa kwanza.</p>
                </div>
            )}
        </div>
        ...
        <div className="bg-[var(--color-surface)] p-5 rounded-2xl shadow-sm border border-[var(--color-border)]">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <InfoItem icon={<Heart />} label="Hali ya Ndoa"
                  value={maritalLabel(congregant.marital_status) || 'Haijulikani'} />
                ...
                <InfoItem icon={<Phone />} label="Namba ya Simu" value={formatPhoneDisplay(congregant.phone)} />
                <InfoItem icon={<Users />} label="Watoto" value={congregant.children_count} />
                {/* Only for members enrolled before phone auth. New records have
                    no address, and a row reading "Barua pepe: -" on every one of
                    them is noise pretending to be a missing field. */}
                {congregant.email && (
            ...
            {contributions.length > 0 ? (
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-[var(--color-border)] text-[12px] text-[var(--color-text-muted)] uppercase font-black">
                        ...
                        <tbody className="divide-y divide-[var(--color-border)]">
                            {[...contributions].reverse().map((c) => (
                                <tr key={c.id} className="text-xs hover:bg-[var(--color-surface-2)] transition-colors">
                                    ...
                                    <td className="py-3 font-bold">
                                        <span className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-bold uppercase ${
                                            c.type?.toLowerCase() === 'zaka' ? 'tag tag-zaka' :
                                            c.type?.toLowerCase() === 'sadaka' ? 'tag tag-sadaka' :
                                            'tag tag-other'
                                        }`}>
                                            {c.type || 'Nyingine'}
                                        </span>
      ...
      <div className="px-4 mt-8 pb-10">
          <div className="bg-[var(--color-surface)] rounded-2xl shadow-sm border border-[var(--color-border)] p-4 flex justify-around">
              <button onClick={() => navigate(`/record-contribution?congregantId=${id}`)} className="flex flex-col items-center gap-1 text-green-600"><Plus size={20} /> <span className="text-[12px]">Ongeza Mchango</span></button>
              <button className="flex flex-col items-center gap-1 text-purple-900"><PencilLine size={20} /> <span className="text-[12px]">Hariri</span></button>
              <button className="flex flex-col items-center gap-1 text-blue-600"><FileText size={20} /> <span className="text-[12px]">Ripoti</span></button>
              <button onClick={handleDelete} className="flex flex-col items-center gap-1 text-[var(--color-rose-on)]"><Trash2 size={20} /> <span className="text-[12px]">Futa</span></button>
          </div>
      </div>
