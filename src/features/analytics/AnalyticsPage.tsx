  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // No auto-selection here. It used to force `selectedChurch` to the
        // first church whenever it was null — but null is exactly what
        // "Makanisa Yote" sets, and this effect re-runs on that change. So
        // choosing "all churches" immediately snapped back to church #1 and
        // the filter looked broken. Null now means all churches the signed-in
        // user is allowed to see, which is what the else branch below applies.
        ...
        // Deliberately NOT guarded by `contributions.length > 0`. It used to be,
        // and with no `else` branch none of the setters below ran when a church
        // had nothing in the chosen period — so the previous church's figures
        // stayed on screen and the filter looked like it was ignoring the
        // selection. Running the aggregation over an empty array yields zeros,
        // which is the honest answer, and keeps one code path instead of a
        // reset block that has to be kept in step with this one.
        let totalZaka = 0;
        let totalSadaka = 0;
        let otherTotal = 0;
        ...
        <div className="flex flex-col gap-2 pb-2 w-full relative z-20">
          <div className="flex items-center gap-1.5 w-full">
            <div className="flex items-center gap-1 flex-1 min-w-0 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[10px] px-2.5 py-2 shadow-sm">
              <Calendar size={12} className="text-[var(--color-teal-on)] shrink-0" />
              <select
                value={timeRange}
                onChange={(e) => setTimeRange(e.target.value)}
                className="w-full text-[12px] font-semibold text-[var(--color-text-dim)] bg-transparent focus:outline-none cursor-pointer"
              >
            ...
            {profile?.role === 'mhazini' ? (
              <div className="flex items-center justify-center gap-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[10px] px-3 py-2 text-[12px] font-semibold text-[var(--color-text-dim)] shadow-sm shrink-0 min-w-[95px] sm:min-w-[115px]">
              ...
              <select 
                ...
                className="flex items-center justify-between gap-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[10px] px-2 py-2 text-[12px] font-semibold text-[var(--color-text-dim)] shadow-sm shrink-0 min-w-[95px] sm:min-w-[115px]"
              >
          ...
          {timeRange === 'custom' && (
            <div className="flex items-center gap-2 bg-purple-50 p-2.5 rounded-[12px] border border-purple-100 flex-wrap sm:flex-nowrap shadow-inner animate-fadeIn">
              <div className="flex flex-col flex-1 min-w-[110px]">
                <span className="text-[11px] font-bold text-[var(--color-text-muted)] mb-0.5 uppercase tracking-wider">Kuanzia Tarehe</span>
                <input 
                  type="date"
                  value={customStartDate}
                  onChange={(e) => setCustomStartDate(e.target.value)}
                  className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[8px] px-2 py-1 text-[12px] font-medium focus:outline-none focus:border-[#00C9A7] w-full"
                />
              </div>
              <div className="flex flex-col flex-1 min-w-[110px]">
                <span className="text-[11px] font-bold text-[var(--color-text-muted)] mb-0.5 uppercase tracking-wider">Hadi Tarehe</span>
                <input 
                  type="date"
                  value={customEndDate}
              ...
              {(customStartDate || customEndDate) && (
                <button 
                  ...
                  className="self-end bg-gray-200 hover:bg-gray-300 text-[var(--color-text-dim)] font-bold px-2.5 py-1 text-[11px] uppercase tracking-wider rounded-md"
                >
                  Futa
                </button>
              )}
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3 mb-2">
                  ...
                  <div className="bg-[var(--color-surface)] rounded-[16px] p-3 shadow-sm border border-[var(--color-border)] flex flex-col gap-2 relative overflow-hidden flex-1">
                    ...
                    <div className="w-8 h-8 rounded-full bg-[#F3E8FF] text-[var(--color-teal-on)] flex items-center justify-center mb-1">
                      <Church size={16} />
                    </div>
                    <div>
                      <p className="text-[12px] text-[var(--color-text-muted)] font-medium leading-none mb-1.5">Jumla ya Mchango</p>
                      <p className="text-[14px] font-bold text-[var(--color-text)] leading-none mb-2">TZS {metrics.jumla.toLocaleString()}</p>
                      <p className="text-[11px] font-medium text-[var(--color-text-muted)] flex items-center gap-0.5">
                        Data halisi
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-[var(--color-surface)] rounded-[16px] p-3 shadow-sm border border-[var(--color-border)] flex flex-col gap-2 relative overflow-hidden flex-1">
                    ...
                    <div>
                      <p className="text-[12px] text-[var(--color-text-muted)] font-medium leading-none mb-1.5">Zaka (Jumla)</p>
                  ...
                  <div className="bg-[var(--color-surface)] rounded-[16px] p-3 shadow-sm border border-[var(--color-border)] flex flex-col gap-2 relative overflow-hidden flex-1">
                    ...
                    <div>
                      <p className="text-[12px] text-[var(--color-text-muted)] font-medium leading-none mb-1.5">Sadaka (Jumla)</p>
                  ...
                  >
                    ...
                    <div>
                      <p className="text-[12px] text-[var(--color-text-muted)] font-medium leading-none mb-1.5">Michango Mingine</p>
                      <p className="text-[14px] font-bold text-[var(--color-text)] leading-none mb-2">
                        TZS {(metrics as any).others?.toLocaleString() || '0'}
                      </p>
                      <p className="text-[11px] font-bold text-[#0074DB] flex items-center gap-0.5">
                  ...
                  <div className="bg-blue-50 rounded-[16px] p-4 shadow-sm border border-blue-100 flex flex-col gap-3 animate-fadeIn">
                    <div className="flex justify-between items-center pb-1 border-b border-blue-200">
                      ...
                      <button 
                        ...
                        className="text-[12px] text-blue-800 hover:text-blue-900 font-bold px-1.5 py-0.5 rounded hover:bg-blue-100"
                      >
                        Funga ✕
                      </button>
                    </div>
                        ...
                        otherContributionsList.map((oc, idx) => (
                          <div key={idx} className="flex justify-between items-center text-[11px] p-2 bg-[var(--color-surface)] rounded-lg hover:shadow-sm transition-shadow">
                            <div className="flex flex-col">
                              ...
                              <span className="text-[11px] text-[var(--color-text-muted)]">{oc.count} michango iliyorekodiwa</span>
                            </div>
                            <span className="font-mono font-bold text-[#0074DB]">TZS {oc.total.toLocaleString()}</span>
                          </div>
                        ))
                      ) : (
                        <p className="text-center text-[12px] text-blue-500 py-3">
                              ...
                              content={({ viewBox }) => {
                                ...
                                return (
                                  <g textAnchor="middle" x={cx} y={cy}>
                                    <text x={cx} y={cy - 8} className="text-[12px] font-bold fill-gray-900">TZS</text>
                                    ...
                                    <text x={cx} y={cy + 18} className="text-[11px] font-medium fill-gray-500">Jumla</text>
                                  </g>
                                );
                              }}
                            />
                    ...
                    <div className="flex flex-col gap-1 w-full mt-1">
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[11px] font-bold text-[var(--color-text)]">Zaka</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[11px] font-bold text-[var(--color-text)]">Sadaka</span>
                        </div>
                        <p className="text-[11px] text-[var(--color-text-muted)] font-medium">
                  ...
                  <div className="bg-[var(--color-surface)] rounded-[16px] p-3 shadow-sm border border-[var(--color-border)] flex flex-col">
                    ...
                    <div className="flex flex-col gap-3 justify-center flex-1">
                      ...
                      {Object.keys((metrics as any).kikundiData || {}).length === 0 && (
                        <p className="text-[12px] text-[var(--color-text-muted)] text-center py-4">Hakuna data ya vikundi.</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[var(--color-surface)] rounded-[16px] p-4 shadow-sm border border-[var(--color-border)] flex flex-col mb-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[12px] p-3 flex flex-col shadow-sm h-[120px] justify-between relative overflow-hidden flex-1">
                      <div className="absolute top-0 left-0 w-1 h-full bg-[var(--color-surface)]" />
                      <div>
                        <div className="text-[var(--color-teal-on)] mb-2">
                          <FileText size={18} />
                        </div>
                        <h3 className="text-[11px] font-bold text-[var(--color-text)] mb-1">Ripoti ya Mwezi</h3>
                        <p className="text-[11px] text-[var(--color-text-muted)] leading-tight pr-2">Tazama ripoti ya mwezi huu</p>
                      </div>
                      <button className="text-[12px] font-bold text-[var(--color-teal-on)] text-left mt-2 self-start flex items-center pr-2">
                    ...
                    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[12px] p-3 flex flex-col shadow-sm h-[120px] justify-between relative overflow-hidden flex-1">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-[var(--color-text-muted)] leading-tight pr-2">Tazama ripoti ya robo mwaka</p>
                      </div>
                      <button className="text-[12px] font-bold text-[#08A452] text-left mt-2 self-start flex items-center pr-2">
                        Tazama <ChevronDown size={12} className="-rotate-90 ml-0.5"/>
                      </button>
                    </div>
                    ...
                    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[12px] p-3 flex flex-col shadow-sm h-[120px] justify-between relative overflow-hidden flex-1">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-[var(--color-text-muted)] leading-tight pr-2">Tazama ripoti ya mwaka mzima</p>
                      </div>
                      <button className="text-[12px] font-bold text-[#F37B1D] text-left mt-2 self-start flex items-center pr-2">
                        Tazama <ChevronDown size={12} className="-rotate-90 ml-0.5"/>
                      </button>
                    </div>
                    ...
                    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[12px] p-3 flex flex-col shadow-sm h-[120px] justify-between relative overflow-hidden flex-1">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-[var(--color-text-muted)] leading-tight pr-2">Pakua ripoti kama PDF / Excel</p>
                      </div>
                      <button className="text-[12px] font-bold text-[#0074DB] text-left mt-2 self-start flex items-center pr-2">
                  ...
                  {churchRecords.map((record) => (
                    <div key={record.id} className="border border-[var(--color-border)] bg-[var(--color-surface)]/30 rounded-[12px] p-3 flex flex-col gap-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[12px] font-medium px-2 py-0.5 bg-purple-50 text-[var(--color-teal-on)] rounded-full">
                          {record.contributorsCount.size} Washiriki waliochangia
                        </span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 mt-1">
                        <div className="flex flex-col">
                          <span className="text-[11px] text-[var(--color-text-muted)]">Jumla ya Zaka</span>
                          <span className="text-[11px] font-mono font-bold text-[var(--color-text)]">TZS {record.zaka.toLocaleString()}</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[11px] text-[var(--color-text-muted)]">Jumla ya Sadaka</span>
                          <span className="text-[11px] font-mono font-bold text-[var(--color-text)]">TZS {record.sadaka.toLocaleString()}</span>
                        </div>
                        <div className="flex flex-col items-end">
                          <span className="text-[11px] text-[var(--color-text-muted)]">Jumla Kuu</span>
                          <span className="text-[11px] font-mono font-bold text-[var(--color-teal-on)]">TZS {record.total.toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
                  ))}
