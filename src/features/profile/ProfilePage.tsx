...
import { LogOut, User, Mail, Phone, Shield, ChevronRight, Settings, Moon, Sun, Edit2, Check, X, Calendar, Receipt } from 'lucide-react';
import { useTheme } from '../../ThemeContext';
import { formatPhoneDisplay } from '../../lib/phone';
import { InstallButton, useInstallAvailable } from '../../components/InstallPrompt';
...
export default function ProfilePage() {
  ...
  // The banner can be dismissed; this entry cannot. Hidden once installed.
  ...
  const handleSaveName = async () => {
    ...
    setSaveError('');
    try {
      // Update, not upsert, and it touches full_name only.
      //
      // This used to upsert the whole row including `role: profile?.role ||
      // 'pastor'`, so renaming yourself rewrote your role — and if `profile`
      // had not loaded, a treasurer editing their name silently became a
      // pastor. Changing a display name must never touch privilege. The row is
      // guaranteed to exist by the time this screen renders (AuthContext
      // creates it), so an update is sufficient.
  ...
  return (
    <div style={{background:'var(--color-ink)',minHeight:'100%',fontFamily:'Noto Sans, sans-serif', transition: 'background 0.3s ease'}}>
      ...
      <div style={{padding:'32px 16px 24px',background:'linear-gradient(180deg, var(--color-ink-800) 0%, var(--color-ink) 100%)',display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center', transition: 'background 0.3s ease'}}>
        <div style={{width:72,height:72,borderRadius:22,background:`color-mix(in srgb, ${roleColor} 20%, transparent)`,border:`2px solid color-mix(in srgb, ${roleColor} 40%, transparent)`,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:14,position:'relative'}}>
          ...
          <div style={{position:'absolute',bottom:-4,right:-4,width:20,height:20,borderRadius:6,background:`${roleColor}`,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <Shield size={11} color="var(--color-on-accent)"/>
          </div>
        </div>
      ...
      <div style={{padding:'0 16px 96px',display:'flex',flexDirection:'column',gap:12}}>
        ...
        <div style={{background:'var(--color-surface)',border:'1px solid var(--color-border)',borderRadius:16,overflow:'hidden'}}>
          <div style={{padding:'14px 16px',borderBottom:'1px solid var(--color-border)',display:'flex',alignItems:'center',gap:12}}>
            <div style={{width:34,height:34,borderRadius:10,background:'color-mix(in srgb, var(--color-sky) 15%, transparent)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
              {user?.phone ? <Phone size={16} color="var(--color-sky)"/> : <Mail size={16} color="var(--color-sky)"/>}
            </div>
            {/* Whichever identifier this account actually signs in with. An
                account created by phone has no address, so a hard-coded
                "Barua Pepe" row would be an empty label on every new account. */}
            <div>
              <p style={{fontSize:12,color:'var(--color-text-muted)',fontFamily:'Sora, sans-serif',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:2}}>
                {user?.phone ? 'Namba ya Simu' : 'Barua Pepe'}
              </p>
              <p style={{fontSize:13,color:'var(--color-text)',fontFamily:'Sora, sans-serif',fontWeight:500}}>
          ...
          <div style={{padding:'14px 16px',display:'flex',alignItems:'center',gap:12}}>
            ...
            <div>
              <p style={{fontSize:12,color:'var(--color-text-muted)',fontFamily:'Sora, sans-serif',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:2}}>Jukumu</p>
        ...
        {/* Install */}
        {installAvailable && (
          <div style={{background:'var(--color-surface)',border:'1px solid var(--color-border)',borderRadius:16,padding:'14px 16px',display:'flex',alignItems:'center',gap:12}}>
            <img src="/icons/icon-192.png" alt="" width={34} height={34} style={{width:34,height:34,borderRadius:10,flexShrink:0}}/>
            <div style={{flex:1,minWidth:0}}>
              <p style={{fontFamily:'Sora, sans-serif',fontWeight:700,fontSize:13,color:'var(--color-text)',margin:0}}>Weka {APP_NAME} kwenye simu</p>
              <p style={{fontSize:12,color:'var(--color-text-muted)',margin:'2px 0 0',lineHeight:1.35}}>Ifungue kutoka skrini ya nyumbani, kama programu nyingine.</p>
            </div>
            <InstallButton appName={APP_NAME} theme={INSTALL_THEME} />
          </div>
        )}

        {/* Actions */}
        {(profile?.role === 'pastor' || profile?.role === 'mhazini') && (
          <div style={{background:'var(--color-surface)',border:'1px solid var(--color-border)',borderRadius:16,overflow:'hidden', marginBottom: 12}}>
            {[
              ...
              // Open to the treasurer as well: they are usually the one who
              // knows what the church's receipt book actually says.
              {label:'Mipangilio ya Stakabadhi',color:'var(--color-teal)',bg:'color-mix(in srgb, var(--color-teal) 15%, transparent)',icon:<Receipt size={16}/>,action:()=>navigate('/receipt-settings')},
            ].map((item,i,arr)=>(
